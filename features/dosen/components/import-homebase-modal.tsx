...
export function ImportHomebaseDosenModal() {
    const { isOpen, getData, close } = useModalManager();
    const open = isOpen("importHomebaseDosenModal");
    const dosen = getData("importHomebaseDosenModal");
    ...
    const onSubmit = async (values: ImportDosenValues) => {
        ...
        try {
            const response = await fetch('/api/dosen/homebase/import', {
                method: 'POST',
                body: formData
            });
            ...
            close('importHomebaseDosenModal');
        }
    };

    return (
        <>
            <BaseModal open={open} onOpenChange={(v) => !v && close("importHomebaseDosenModal")} size="md">
                <BaseModal.Header>
                    <BaseModal.Title>Import Homebase Dosen</BaseModal.Title>
                ...
                <BaseModal.Footer>
                    <BaseModal.CloseButton onClick={() => close("importHomebaseDosenModal")} />
                    <Button
                        type='submit'
                        onClick={form.handleSubmit(onSubmit)}
                        disabled={isSubmitting}
                    >
                        {isSubmitting ? 'Processing..' : dosen ? 'Simpan Perubahan' : 'Process'}
                    </Button>
                </BaseModal.Footer>
            </BaseModal>
        </>
